...
export function Nav() {
  return (
    <nav className="sticky top-0 z-30 flex items-center justify-between px-6 md:px-10 py-5 md:py-6 bg-[var(--color-bg)]/92 backdrop-blur-md border-b border-[var(--color-accent)]/15 overflow-hidden">
      ...
      {/* Center links — numbered + display font + animated underline */}
      <div className="relative z-10 hidden md:flex gap-7 lg:gap-9">
        {links.map((l) => (
          <Link
            key={l.href}
            href={l.href}
          ...
          >
            <span className="font-[family-name:var(--font-mono)] text-[10.5px] tracking-[0.18em] text-[var(--color-accent-cyan)] opacity-75 group-hover:opacity-100 transition-opacity">
              {l.num}
            </span>
            <span className="font-[family-name:var(--font-display-alt)] font-bold text-[15px] md:text-[16px] uppercase tracking-[0.05em] text-[var(--color-fg)] group-hover:text-[var(--color-accent)] transition-colors">
              {l.label}
            </span>
            <span className="nav-underline absolute -bottom-1 left-[26px] right-0 h-px bg-[var(--color-accent)] origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out" />
          </Link>
        ))}
      </div>
      ...
      <a
        ...
        download
        className="resume-pill relative z-10 group inline-flex items-center gap-2 font-[family-name:var(--font-mono)] text-[11.5px] md:text-[12.5px] tracking-[0.2em] uppercase font-semibold px-5 py-2.5 rounded-full border border-[var(--color-accent)]/80 text-[var(--color-accent)] bg-[var(--color-bg)]/85 hover:bg-[var(--color-accent)] hover:text-[var(--color-bg)] hover:shadow-[0_0_24px_-4px_var(--color-accent)] transition-all duration-300"
      >
        <span>Resume</span>
